...
void main () {
    gl_Position = vec4(point_data.x * u_scale.x, point_data.y * u_scale.y, 0, 1);
...
class SignalVisualizer(AlsaCanvas):
    ...
    def on_data(self, event):
        ...
        if len(all_data) > 0:
            ...
            point_data = np.stack((x_data, all_data),
                                  axis=-1).astype(np.float32)
            self.audio_signal_program['point_data'] = point_data

            data = self.pcm_buffer.data()
            tempo, beat_indices = beat.beat_track(
                y=data, sr=self.sample_rate, units='samples')
            beats = (beat_indices * 2 - beat_indices) / samples
